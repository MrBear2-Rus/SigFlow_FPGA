...
std::string TopNode::ToVerilog() {
    std::string v = "module " + identifier;
    v += "(";
    ...
    for (size_t i = 0; i < allPorts.size(); ++i) {
        v += Port::portDirectionToStr(allPorts[i]->direction) + " " + allPorts[i]->identifier;
...
std::string ModuleInstNode::ToVerilog() {
    std::string v = "    " + defIdentifier + " " + identifier;
    v += "(\n";
    // 合并 in/out 端口列表以正确处理末尾逗号
    for (auto port : in_ports) {
        v += "        ." + port.identifier + "(" + port.conn + "),\n";
    }
    for (auto port : out_ports) {
        v += "        ." + port.identifier + "(" + port.conn + "),\n";
    }
