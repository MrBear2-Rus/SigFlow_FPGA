...
typedef struct {
    ...
    char module_path[VCD_NAME_SIZE]; // 新增：存储信号所属模块路径（如TOP/TOP.full_adder）
    char full_name[VCD_NAME_SIZE]; // 新增：存储完整信号名（如TOP.a/TOP.full_adder.u1.sum）
...
typedef struct {
    ...
    char current_module_path[VCD_NAME_SIZE]; // 新增：解析时记录当前模块路径（如进入TOP→TOP.full_adder）
} vcd_t;
