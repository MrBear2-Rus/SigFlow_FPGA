...
// 跨平台 HTTP 走 wxWebRequest：Windows 用 WinHTTP（自带），Linux/macOS 用 libcurl。
// Linux 上若 wx 构建时没找到 libcurl，wxUSE_WEBREQUEST 会是 0 —— 此时插件无法工作，
// 给出明确报错（解决：安装 libcurl 开发包后重新构建 wxWidgets）。
#if !wxUSE_WEBREQUEST
#error "DeepSeek 插件需要 wxWidgets 启用 wxUSE_WEBREQUEST（Linux 需 libcurl 后重建 wx）"
#endif
...
namespace {
// API Key 不再硬编码（原先明文入源码/git，属安全事件）。
// 解析顺序：① 环境变量 SIGFLOW_DEEPSEEK_API_KEY
//           ② 用户数据目录下的 deepseek.key（一行）
std::string ResolveDeepSeekApiKey() {
    if (const char* env = std::getenv("SIGFLOW_DEEPSEEK_API_KEY"); env && env[0] != '\0') {
        return std::string(env);
    }
    try {
        const wxString dataDir = wxStandardPaths::Get().GetUserLocalDataDir();
        if (!dataDir.IsEmpty()) {
            std::ifstream keyFile((std::filesystem::path(dataDir.ToStdString()) / "deepseek.key").string());
            if (keyFile) {
                std::string key;
...
Plug_DeepSeek::Plug_DeepSeek() {
    m_apiKey = ResolveDeepSeekApiKey();
    m_apiUrl = "https://api.deepseek.com/chat/completions";
    m_projectRoot = "";

    // 历史对话保存在用户数据目录（Windows=%LOCALAPPDATA%，Linux=~/.local/share 等），
    // 无需管理员权限且对多用户环境友好。
    const wxString dataRoot = wxStandardPaths::Get().GetUserLocalDataDir();
    const std::filesystem::path dataDir = dataRoot.IsEmpty()
        ? std::filesystem::current_path() / ".sigflow" / ".pluginDeepSeek"
        : std::filesystem::path(dataRoot.ToStdString()) / "pluginDeepSeek";
    m_dataDir = dataDir.string();
    m_historyFile = (dataDir / "history.json").string();
...
// 在当前线程同步执行一次 HTTPS POST。
// 依赖 wxWebRequest（Windows=WinHTTP，Linux=libcurl），需要当前线程能跑 wxEventLoop：
// 调用方是插件的工作线程，这里用局部 wxEventLoop + Dispatch 泵事件。
HttpOutcome PostJsonSync(const wxString& url, const wxString& bearerToken,
                         const std::string& jsonBody, int timeoutMs,
                         const std::atomic<bool>& cancelFlag) {
    HttpOutcome out;

    // wx 3.2 的默认 session 在首次 CreateRequest 时自动打开。
    wxWebSession& session = wxWebSession::GetDefault();

    wxEvtHandler handler;
    wxWebRequest request = session.CreateRequest(&handler, url);
    if (!request.IsOk()) {
        out.error = "Error: unable to create web request.";
        return out;
    }
    ...
    handler.Bind(wxEVT_WEBREQUEST_STATE, [&](wxWebRequestEvent& evt) {
        ...
        case wxWebRequest::State_Completed:
            out.status = evt.GetResponse().GetStatus();
            out.body = std::string(evt.GetResponse().AsString().ToUTF8().data());
            done = true;
            break;
        case wxWebRequest::State_Failed:
            out.error = "Error: " + std::string(evt.GetErrorDescription().ToUTF8().data());
            done = true;
            break;
    ...
    const wxLongLong startMs = wxGetUTCTimeMillis();
    while (!done) {
        loop.Dispatch();
        if (done) break;
        if (cancelFlag.load()) { request.Cancel(); break; }
        if (timeoutMs > 0 && (wxGetUTCTimeMillis() - startMs) > timeoutMs) { request.Cancel(); break; }
        wxMilliSleep(2);
    }
    if (!done) {
        // 取消/超时后把剩余事件泵干净，避免 handler 析构时仍有事件投递
        for (int i = 0; i < 20 && !done; ++i) { loop.Dispatch(); wxMilliSleep(2); }
        if (out.error.empty() && out.body.empty()) out.error = "Error: request timed out or cancelled.";
    }
    return out;
}

// 从非流式响应体里取 choices[0].message.content
std::string ExtractMessageContent(const std::string& body) {
    try {
...
// 从 SSE（stream=true）响应体里拼接所有 delta.content
std::string AssembleStreamContent(const std::string& body,
                                  const std::function<void(const std::string&)>& onDelta) {
    std::string assembled;
    std::string sseBuf = body;
    size_t pos = 0;
    while ((pos = sseBuf.find("\n\n")) != std::string::npos) {
        const std::string event = sseBuf.substr(0, pos);
        sseBuf.erase(0, pos + 2);
        std::istringstream iss(event);
        std::string line, dataStr;
        ...
        try {
            ...
            if (j.contains("choices") && j["choices"].is_array() && !j["choices"].empty()) {
                ...
                if (ch.contains("delta") && ch["delta"].contains("content")) {
                    const std::string delta = ch["delta"]["content"].get<std::string>();
                    assembled += delta;
...
std::string Plug_DeepSeek::CallDeepSeekAPI(const std::string& prompt, wxWindow* panel, bool stream) {
    if (m_apiKey.empty()) {
        return std::string("DeepSeek API Error: 未配置 API Key。请设置环境变量 "
                           "SIGFLOW_DEEPSEEK_API_KEY，或写入用户数据目录下的 deepseek.key。");
    }
    ...
    const std::string jsonBody = payload.dump();
    ...
    for (int attempt = 1; attempt <= maxRetries && !m_cancelRequest; ++attempt) {
        responseData.clear();
        const HttpOutcome outcome = PostJsonSync(
            wxString::FromUTF8(m_apiUrl), wxString::FromUTF8(m_apiKey), jsonBody,
            120000, m_cancelRequest);
        statusCode = outcome.status;
        responseData = outcome.body;

        if (outcome.cancelled || m_cancelRequest) {
            m_requestInProgress = false;
            return std::string("Error: Request cancelled by user.");
        }
        if (!outcome.error.empty()) {
            responseData = outcome.error;
            if (attempt < maxRetries) { wxMilliSleep(500 * (1 << (attempt - 1))); continue; }
            break;
        }
        if (statusCode == 401) {
            m_requestInProgress = false;
            return std::string("DeepSeek API Error: Unauthorized (401). Check API key.");
        }
    ...
    // wxWebRequest 不提供逐块回调，流式降级为完成后一次性回传（内容与原先一致）。
    std::string finalStr;
    if (stream) {
        const std::string assembled = AssembleStreamContent(responseData, nullptr);
        finalStr = !assembled.empty() ? assembled : ExtractMessageContent(responseData);
    } else {
        finalStr = ExtractMessageContent(responseData);
    }

    if (panel && stream) {
        if (!finalStr.empty()) {
            wxThreadEvent* partEvt = new wxThreadEvent(EVT_AI_RESPONSE);
            partEvt->SetString(wxString::FromUTF8(finalStr));
            partEvt->SetInt(1);
            wxQueueEvent(panel, partEvt);
        }
        wxThreadEvent* finalEvt = new wxThreadEvent(EVT_AI_RESPONSE);
        finalEvt->SetString(wxString::FromUTF8(finalStr.empty() ? responseData : finalStr));
        finalEvt->SetInt(2);
        wxQueueEvent(panel, finalEvt);
    }

    if (!finalStr.empty()) return finalStr;

    try {
        auto resJson = json::parse(responseData);
        if (resJson.contains("error") && resJson["error"].contains("message")) {
...
void Plug_DeepSeek::CancelCurrentRequest() {
    // 只置取消标志：工作线程在事件泵里检测到后会 request.Cancel()。
    m_cancelRequest = true;
}

extern "C" SIGFLOW_PLUGIN_EXPORT ISigPlugin* CreateSigPlugin() {
